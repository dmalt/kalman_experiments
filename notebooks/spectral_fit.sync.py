...
from kalman_experiments.models import ArNoiseModel, MatsudaParams, SingleRhythmModel
...
mp = MatsudaParams(A=0.99999, freq=6, sr=SRATE)
# mp = MatsudaParams(A=1, freq=6, sr=SRATE)

est_psd_func = partial(get_psd_val_from_est, freqs=ff, psd=psd)
mar_psd_func = SingleRhythmModel(mp, sigma=1).psd_onesided

noise_model_white = ArNoiseModel(np.array([0]), sr=mp.sr, order=1, alpha=0, sigma=1)
noise_model_pink = ArNoiseModel(np.array([0] * 30), sr=mp.sr, order=30, alpha=1.5, sigma=1)

ar_noise_psd_func_pink = noise_model_pink.psd_onesided
ar_noise_psd_func_white = noise_model_white.psd_onesided
